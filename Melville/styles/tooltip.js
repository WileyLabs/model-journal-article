...
function tooltip_create(event)
{
	...
	else
	{
		...
		if ($(this).is('.term-label'))
		{
			...
			var displayForm = ($(this).prev().parent('.termGroup').children('.type-displayForm').length) ? $(this).prev().parent('.termGroup').children('.type-displayForm').html() : ($(this).children('.term').length) ? $(this).children('.term').html() : $(this).html();
