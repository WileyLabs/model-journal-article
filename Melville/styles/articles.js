...
function lineNumberingColon()
{
	var labels = document.querySelectorAll('.computerCode>.lineatedText>.line:not([data-count-line="no"])>.generated-labeling');
	for (var i=0; i<labels.length; i++) {
		if (labels[i].textContent.match(/\d+/i )) {
			labels[i].textContent = labels[i].textContent.replace(/(.*)/, '$1:');
		}
	}
}
